...
type Props = {
  ...
  campusName: string;
  article: ApiArticle;
  primaryRelatedArticles: ApiArticle[];
  latestPublishedArticles: ApiArticle[];
...
export default function ArticlePageClient({
  ...
  const relatedArticles = useMemo(() => {
    ...
    return merged;
  }, [apiArticle.slug, primaryRelatedArticles, latestPublishedArticles]);
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 w-full min-w-0">
        <nav className="flex items-center text-black text-sm mb-6 flex-wrap gap-1">
          ...
          <Link href={`/campus/${campusSlug}`} className="hover:text-[#991b1b]">{campusName}</Link>
          <ChevronRight className="h-4 w-4 mx-1" />
        ...
        {apiArticle.body ? (
          <article className="prose prose-lg max-w-none mb-8" dangerouslySetInnerHTML={{ __html: stripImageCardsFromHtml(apiArticle.body) }} />
        ) : (
