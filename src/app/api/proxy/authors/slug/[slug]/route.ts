...
export async function GET(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  const upstreamUrl = new URL(
    `${API_BASE.replace(/\/$/, '')}/api/authors/slug/${encodeURIComponent(slug)}/`
  );
  ...
  articlesUrl.searchParams.set('author_username', slug);
  ...
  try {
    const badgeRes = await fetch(
      `${API_BASE.replace(/\/$/, '')}/api/profiles/badge/${encodeURIComponent(slug)}/`,
