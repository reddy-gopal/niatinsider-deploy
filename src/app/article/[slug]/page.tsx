...
export default function Article() {
  ...
  const articleSlugParam = slug?.trim() || null;
  ...
  const handleDeleteArticle = () => {
    ...
    articleService
      ...
      .then(() => {
        ...
        router.push(isGlobalRoute ? '/articles' : `/campus/${campusSlug ?? ''}`);
      })
    ...
    return (
      <div className="min-h-screen bg-white overflow-x-hidden">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <Link href={isGlobalRoute ? '/articles' : `/campus/${campusSlug ?? ''}`} className="text-[#991b1b] font-medium hover:underline">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 w-full min-w-0">
        ...
        <nav className="flex items-center text-black text-sm mb-6 flex-wrap gap-1">
          ...
          {isGlobalRoute ? (
            ...
            <>
              <Link href={`/campus/${campusSlug ?? ''}`} className="hover:text-[#991b1b]">
                {campus?.name ?? 'Campus'}
              </Link>
              <ChevronRight className="h-4 w-4 mx-1" />
              <span
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          {isAuthor && (
            <div className="relative shrink-0" ref={optionsRef}>
              <button type="button" onClick={(e) => { e.stopPropagation(); setOptionsOpen((v) => !v); }} className="p-2 rounded-lg text-[#64748b] hover:bg-[rgba(30,41,59,0.08)] hover:text-[#1e293b] transition-colors" aria-label="Article options" aria-expanded={optionsOpen}>
              ...
              {optionsOpen && (
                <div className="absolute right-0 top-full mt-1 py-1 min-w-[180px] rounded-lg bg-white border shadow-lg z-10" style={{ borderColor: 'rgba(30,41,59,0.12)' }}>
                  <Link href={`/contribute/write?edit=${apiArticle!.id}`} className="flex items-center gap-2 w-full px-4 py-2.5 text-left text-sm text-[#1e293b] hover:bg-[#fbf2f3]" onClick={() => setOptionsOpen(false)}>
        ...
        {loading && <div className="flex justify-center py-16 mb-8"><div className="animate-spin rounded-full border-2 border-[#fbf2f3] size-10 border-t-[#991b1b]" role="status" aria-label="Loading" /></div>}
        ...
        {!loading && (
          <>
            {fromApi && apiArticle?.body ? (
              <div className="article-body-read-only"><article className="prose prose-lg max-w-none mb-8" dangerouslySetInnerHTML={{ __html: stripImageCardsFromHtml(apiArticle.body) }} /></div>
            ) : (
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-black mb-6 pb-6 border-t border-[rgba(30,41,59,0.1)] pt-6">
              <span>Written by {article.author}</span><span className="flex items-center"><Clock className="h-4 w-4 mr-1" />Last updated {article.updatedDays} days ago</span><span className="flex items-center gap-1"><ThumbsUp className="h-4 w-4" />{displayUpvoteCount} upvote{displayUpvoteCount !== 1 ? 's' : ''}</span><span className="flex items-center gap-1"><Eye className="h-4 w-4" />{displayViewCount} view{displayViewCount !== 1 ? 's' : ''}</span>
            </div>
